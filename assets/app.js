...
// Origen de una cancelación (solo para el grupo "cancelado"). Tres categorías,
// pensadas para saber a quién reclamar:
//  - "interna": cancelada por el vendedor / servicio al cliente, sin intento de
//    entrega (típico CANCELADO / GUIA_ANULADA en "pendiente confirmación").
//  - "cliente": el destinatario rechazó, no estaba o se arrepintió (COD). NO es
//    reclamable a la transportadora.
//  - "transportadora": falla logística (dirección inubicable, no entregado,
//    extravío, daño, demora) o devolución en ruta sin motivo -> reclamable a la
//    transportadora (carrier): pedir pronunciamiento / indemnización a Dropi.
function cancelOrigin(r) {
  if (r.grupo !== "cancelado") return null;
  const e = norm(r.estado);
  const nv = norm(r.novedad || "");
  if (nv) {
    if (/RECHAZ|CLIENTE|DESTINATARIO|NADIE EN CASA|NO ESTABA|SE ARREPINT|NO QUIERE|NO CONTESTA|NO RESPONDE/.test(nv)) return "cliente";
    if (/INUBICABLE|COBERTURA|ZONA|NO ENTREG|EXTRAVI|PERDID|SINIESTR|DANAD|DETERIOR|DEMORA|AVERIAD|NO SE PUDO/.test(nv)) return "transportadora";
    return "cliente"; // novedad con motivo no logístico -> se atribuye al cliente (conservador)
  }
  if (e.includes("DEVOLUCION")) return "transportadora"; // devuelto en ruta sin motivo
  return "interna";
}
...
function computeMetrics(rows) {
  ...
  const cancelInterna = { n: 0, rec: 0 };        // vendedor / servicio al cliente (sin intento)
  const cancelCliente = { n: 0, rec: 0 };        // el cliente rechazó / no estaba / se arrepintió
  const cancelTransportadora = { n: 0, rec: 0 }; // falla logística reclamable (carrier)
  ...
  for (const r of rows) {
    ...
    if (r.grupo === "cancelado") {
      ...
      const bucket = org === "transportadora" ? cancelTransportadora
        : org === "cliente" ? cancelCliente : cancelInterna;
  ...
  return {
    ...
    cancelInterna, cancelCliente, cancelTransportadora,
...
function buildDecisions(m) {
  ...
  const cc = m.cancelCliente || { n: 0, rec: 0 };
  ...
  // 4a. Cancelaciones internas (vendedor / servicio al cliente): confirmar pedidos.
  if (ci.n > 0) {
    decisions.push({
      ...
      detail: `${fmtInt(ci.n)} canceladas por el vendedor / servicio al cliente (sin intento de entrega), ${fmtMoney(ci.rec)} perdidos.`,
      action: "Confirma el pedido (order confirmation) antes de despachar — con un bot de WhatsApp o llamada. Aquí se frena el grueso de la fuga; NO es reclamable a la transportadora.",
    });
  }

  // 4b. Rechazos del cliente: verificar falsos rechazos + reducir con confirmación.
  if (cc.n > 0) {
    const sosp = (m.rows || []).filter((r) => cancelOrigin(r) === "cliente" && r.dias != null && r.dias <= 1).length;
    decisions.push({
      key: "cancel_cliente", label: "VERIFICAR", tone: sosp > 0 ? "orange" : "yellow",
      title: "Verificar rechazos del cliente (possible false rejections)", impact: cc.rec,
      detail: `${fmtInt(cc.n)} marcadas como rechazo del cliente${sosp > 0 ? `, ${fmtInt(sosp)} de ALTA SOSPECHA (rechazadas en ≤1 día)` : ""} · ${fmtMoney(cc.rec)}.`,
      action: "OJO: el rechazo lo declara el repartidor. Exige a Dropi la prueba del intento (GPS/foto/llamada); los que no la tengan son FALSO RECHAZO (false rejection) = falla de transportadora y SÍ son reclamables. Los reales se reducen confirmando interés y pago antes de despachar.",
    });
  }

  // 4c. Fallas de transportadora: pedir pronunciamiento / indemnización a Dropi.
  if (ct.n > 0) {
    decisions.push({
      key: "cancel_transportadora", label: "RECLAMAR", tone: "orange",
      title: "Reclamar fallas de transportadora (carrier fault)", impact: ct.rec,
      detail: `${fmtInt(ct.n)} con falla logística (dirección inubicable, no entregado, extravío, daño…), ${fmtMoney(ct.rec)} afectados.`,
      action: "Descarga el detalle con el motivo por guía y solicita a Dropi pronunciamiento (ruling) sobre la transportadora: revisión o indemnización (indemnity) del flete y del recaudo.",
    });
  }
...
function cancelOriginLabel(r) {
  ...
  if (o === "interna") return "Interna (vendedor)";
  if (o === "transportadora") return "Transportadora (carrier)";
  if (o === "cliente") {
    // El rechazo lo declara el repartidor: siempre "a verificar". ≤1 día = muy
    // rápido para un intento real -> alta sospecha de falso rechazo.
    return (r.dias != null && r.dias <= 1)
      ? "Cliente rechazó · ALTA SOSPECHA falso rechazo (verify)"
      : "Cliente rechazó · verificar (courier-reported)";
  }
  return "";
}

// Columnas del detalle exportable (sin datos personales).
const DETAIL_COLS = ["Guía", "ID orden", "Estado", "Origen", "Motivo (novedad)", "Grupo", "Transportadora", "Ciudad", "Departamento", "Días", "Recaudo", "Flete"];
function detailRowValues(r) {
  return [r.tracking || "", r.orden || "", r.estado || "", cancelOriginLabel(r), r.novedad || "", r.grupo || "", r.transportadora || "",
...
function detailFor(m, spec) {
  ...
  if (spec.indexOf("carrier:") === 0) {
    ...
    if (byGroup[key]) {
      ...
      note = "Canceladas por el vendedor / servicio al cliente, sin intento de entrega. Se frenan confirmando el pedido antes de despachar.";
    } else if (key === "cancel_cliente") {
      list = rows.filter((r) => cancelOrigin(r) === "cliente");
      title = "Rechazos del cliente — verificar (possible false rejections)";
      note = "El rechazo lo declara el repartidor. Los de ALTA SOSPECHA (≤1 día) exígeselos a Dropi con prueba del intento; sin prueba son falso rechazo (carrier fault) y reclamables.";
    } else if (key === "cancel_transportadora") {
      list = rows.filter((r) => cancelOrigin(r) === "transportadora");
      title = "Fallas de transportadora reclamables (carrier fault)";
      note = "Falla logística (inubicable, no entregado, extravío, daño…). Descarga el detalle y pide a Dropi pronunciamiento / indemnización.";
    } else if (key === "cancel") {
...
function openDetail(spec) {
  ...
  document.getElementById("detailBody").innerHTML = d.list.length
    ...
    : `<tr><td colspan="9">Sin órdenes en esta categoría.</td></tr>`;
