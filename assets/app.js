...
function buildDecisions(m) {
  ...
  // 4b. Rechazos del cliente (COD): reducir con confirmación previa.
  if (cc.n > 0) {
    decisions.push({
      key: "cancel_cliente", label: "CONFIRMAR", tone: "yellow",
      title: "Reducir rechazos del cliente (customer rejections)", impact: cc.rec,
      detail: `${fmtInt(cc.n)} rechazadas por el cliente (rechazó / no estaba / se arrepintió), ${fmtMoney(cc.rec)} perdidos.`,
      action: "Confirma interés y pago antes de despachar; filtra impulsivos. Tampoco es reclamable a la transportadora (es del cliente).",
    });
  }
...
function cancelOriginLabel(r) {
  ...
  return o === "interna" ? "Interna (vendedor)"
    : o === "cliente" ? "Cliente rechazó (customer)"
    : o === "transportadora" ? "Transportadora (carrier)" : "";
}
...
function detailFor(m, spec) {
  ...
  if (spec.indexOf("carrier:") === 0) {
    ...
    if (byGroup[key]) {
      ...
      title = "Rechazos del cliente (customer rejections)";
      note = "El cliente rechazó, no estaba o se arrepintió. NO es reclamable a la transportadora; se reduce con confirmación previa.";
